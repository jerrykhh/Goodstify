...
interface ShopPageProps {
    ...
    imageCDN_endPoint: string

}

const ShopPage = ({ igName, countryCode, categorieId, categories, icon, imageCDN_endPoint }: ShopPageProps) => {
    ...
    const encodeQueryString = () => {
        ...
        if ((encoded === "" || Object.keys(cartState.items).length === 0))
            router.push(`/${countryCode}`, undefined, { shallow: true })
        else
            router.push(`/${countryCode}?q=${encoded}`, undefined, { shallow: true });
    }
    ...
    const convertProductImage2CDN = (product: Product): Product => {
        ...
        cdnImageProduct.image = `${imageCDN_endPoint}${product.image}`;
    ...
    return (
        <React.Fragment>
          <Head>
            <title>Goodstify {countryCode.toUpperCase()}</title>
          ...
          <ShopCartListModalContext.Provider value={{ isOpenCartModal, setIsOpenCartModal }}>
            <ShopCartList igName={igName} countryCode={countryCode} cart={Object.values(cartState.items)} />
          ...
          <div className={`h-screen` + (isOpenCartModal || openProductItem != null ? ' overflow-hidden' : '')}>
            <div className='w-full border-b'>
              <div className='flex justify-center items-center py-4'>
                <div className='text-2xl font-sans'>Goodstify.{countryCode}</div>
              </div>
            </div>
                                ...
                                return (
                                  <div className='card' key={product.id}>
                                    <div className='max-w-sm'>
                                      <img className='object-cover cursor-pointer' src={`${imageCDN_endPoint}${product.image}`} alt={`img-${product.desc}`} onClick={() => setOpenProductItem(convertProductImage2CDN(product))} />
                                    </div>
            ...
            {Object.keys(cartState.items).length > 0 ?
              <div className='fixed bottom-0 border border-b w-full bt-4 pb-6 lg:py-2 px-6 bg-white z-20' ref={checkoutRef}>
